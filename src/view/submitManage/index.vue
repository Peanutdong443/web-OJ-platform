...
<template>
  <div class="info-notice-wrapper">
    ...
    <div class="table-content">
      ...
      >
        ...
        <template slot-scope="scope">
         <span v-if="!scope.row.auditStatus">未检查</span>
         <span v-else-if="scope.row.auditStatus==1">正确</span>
         <span v-else-if="scope.row.auditStatus == 2">失败</span>
        </template>
      </el-table-column>
    ...
    <el-drawer
      title="实验详情"
      :visible.sync="examDetailsVisible"
      :before-close="handleClearExamDetails"
      size="1000px"
      v-loading="examDetailsLoading"
    ...
    <el-drawer
      title="提交作业"
      :visible.sync="submitWorkVisible"
    ...
    async handleSearchSubmitRecord(){
      ...
      for(let i=0;i<res.length;i++){
        ...
        if(item.username==localStorage.getItem("username")){
          this.submitRecordList.push(item)
        }
      }
      console.log("submitRecordList",this.submitRecordList)
    },
    //初始化对话框，重置对话框
